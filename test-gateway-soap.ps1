...
try {
    Write-Host "Testing SOAP request through API Gateway (port 8082)..."
    $response = Invoke-WebRequest -Uri "http://localhost:8082/services/RendezvousService" -Method POST -Headers $headers -Body $soapRequest -UseBasicParsing
    
    Write-Host "✅ Success! Status Code: $($response.StatusCode)"
    Write-Host "CORS Headers:"
    $response.Headers['Access-Control-Allow-Origin'] | ForEach-Object { Write-Host "  Access-Control-Allow-Origin: $_" }
